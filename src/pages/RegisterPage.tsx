 ...
 * Handles user registration with full name, age, email, password, and password confirmation
...
export const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!age) {
      ...
      setAgeError('Ingresa una edad válida');
      hasError = true;
    }
    ...
    try {
      await registerWithEmail(email, password, fullName, parseInt(age));
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-(--color-background)">
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center">
        {/* Register Container - Full screen on mobile with transparent bg, card on desktop */}
          ...
          <div className="flex-1 flex flex-col justify-center px-6 md:px-8 pb-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 mb-6">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full h-12 px-4 bg-(--color-input-bg) border-2 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 ring-(--color-primary) focus:border-transparent transition-all ${
                    fullNameError ? 'border-(--color-error)' : 'border-(--color-border)'
                  }`}
                />
                {fullNameError && (
                  <p className="mt-1 text-sm text-(--color-error)">{fullNameError}</p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full h-12 px-4 bg-(--color-input-bg) border-2 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 ring-(--color-primary) focus:border-transparent transition-all ${
                    ageError ? 'border-(--color-error)' : 'border-(--color-border)'
                  }`}
                />
                {ageError && (
                  <p className="mt-1 text-sm text-(--color-error)">{ageError}</p>
                )}
              </div>
            ...
            <div className="relative mb-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-container text-gray-400">O</span>
              </div>
            </div>
            ...
            <div className="text-center">
              <p className="text-sm text-gray-400">
                ...
                <Link to="/login" className="font-semibold text-white hover:text-gray-300 transition-colors">
                  Inicia Sesión
                </Link>
              </p>
            </div>
